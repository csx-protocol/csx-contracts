...
// }
 ...
 const stickers = [
  ...
  for (let i = 0; i < prices.length; i++) {
    const result = await tradeFactory.createListingContract(names[i], _tradeUrl, '112' + i, inspctLink[i], imgs[i], prices[i], floatInfo[i], stickers[i], weaponTypes[i]);
    console.log('>>>COMPLETED ' + (i+1) + " out of " + prices.length);
  }

  console.log('done ' + TradeFactory.address);
};
