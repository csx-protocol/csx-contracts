...
describe("TradeFactory", function () {
    ...
    const prices = ['2977130000000000', '8933480000000000', '27800000', '44661960000000000', '89323910000000000', '457000000', '1489077140000000000', '1489077140000000000'];
    const names = ['R8 Revolver | Bone Mask (Field-Tested)', 'Negev | Bulkhead (Minimal Wear)', 'Negev | Army Sheen (Minimal Wear)', 'MAC-10 | Nuclear Garden (Field-Tested)', 'Galil AR | Rocket Pop (Factory New)', 'StatTrak™ AK-47 | Slate (Factory New)', '★ Driver Gloves | Rezan the Red (Minimal Wear)', '★ Driver Gloves | Rezan the Red (Minimal Wear)']
    const weaponTypes = ['R8 Revolver', 'Negev', 'Negev', 'MAC-10', 'Galil AR', 'AK-47', 'Driver Gloves', 'Driver Gloves'];
    const imgs = ['https://media.steampowered.com/apps/730/icons/econ/default_generated/weapon_revolver_sp_tape_light_large.c8f9124ff70ca2a6e8867920cd39e4fb7308ac87.png', 'https://media.steampowered.com/apps/730/icons/econ/default_generated/weapon_negev_hy_ducts_yellow_light_large.9d9335325a4a696ec6c2ef704ec1d4b3112c8c87.png', 'https://media.steampowered.com/apps/730/icons/econ/default_generated/weapon_negev_am_army_shine_light_large.884085f4a13b786f0ac7234d616ff01a848f28d5.png', 'https://media.steampowered.com/apps/730/icons/econ/default_generated/weapon_mac10_am_nuclear_skulls3_mac10_light_large.467b325065522e5248247cf125bec257cdb66902.png', 'https://media.steampowered.com/apps/730/icons/econ/default_generated/weapon_galilar_cu_galilar_particles_light_large.8732f64d53dbc9b0c732641655d4f99124d8cacc.png', 'https://media.steampowered.com/apps/730/icons/econ/default_generated/weapon_ak47_gs_ak47_professional_light_large.d09d623d0a725c63e8a3905f66bba41ba2ed59e8.png', 'https://media.steampowered.com/apps/730/icons/econ/default_generated/slick_gloves_slick_rezan_light_large.642934831085e8715a7e8072614f71f9fc0f205e.png', 'https://media.steampowered.com/apps/730/icons/econ/default_generated/slick_gloves_slick_rezan_light_large.642934831085e8715a7e8072614f71f9fc0f205e.png']
    const inspctLink = ['steam://rungame/730/76561202255233023/+csgo_econ_action_preview%20S76561198185748194A28591758742D769815130885352460', 'steam://rungame/730/76561202255233023/+csgo_econ_action_preview%20S76561198185748194A28492787574D16143929557675144168', 'steam://rungame/730/76561202255233023/+csgo_econ_action_preview%20S76561198185748194A28433491397D9821478250864647424', 'steam://rungame/730/76561202255233023/+csgo_econ_action_preview%20S76561198185748194A28315956209D14460612990892731053', 'steam://rungame/730/76561202255233023/+csgo_econ_action_preview%20S76561198185748194A28315874929D12397849034060993453', 'steam://rungame/730/76561202255233023/+csgo_econ_action_preview%20S76561198185748194A28229768155D11574396181669127541', 'steam://rungame/730/76561202255233023/+csgo_econ_action_preview%20S76561198185748194A27955299910D14756333338738051571', 'steam://rungame/730/76561202255233023/+csgo_econ_action_preview%20S76561198185748194A27955299910D14756333338738051571']
    const skinInfo = [
        {
    ...
    const assetIds = ['GG1', '28492787574', 'GG2', '28315956209', '28315874929', '28229768155', '27955299910', '3213213'];
    ...
    describe("Initialization", function () {
        it("should create test listings that return correct data", async function () {
            ...
            expect(totalListings).to.equal(prices.length, "The total number of listing contracts should match the length of prices array");

            // Check if each contract was created with the correct data
            for (let i = 0; i < prices.length; i++) {
                const listing = await tradeFactory.getTradeDetailsByIndex(i);
                
                expect(listing.itemMarketName).to.equal(names[i], `The itemMarketName for listing ${i} should match`);
                expect(listing.sellerTradeUrl.partner).to.equal(_tradeUrl.partner, `The tradeUrl partner for listing ${i} should match`);
                expect(listing.sellerTradeUrl.token).to.equal(_tradeUrl.token, `The tradeUrl token for listing ${i} should match`);
                expect(listing.assetId).to.equal(assetIds[i], `The assetId for listing ${i} should match`);
                expect(listing.inspectLink).to.equal(inspctLink[i], `The inspectLink for listing ${i} should match`);
                expect(listing.itemImageUrl).to.equal(imgs[i], `The itemImageUrl for listing ${i} should match`);
                expect(listing.weiPrice).to.equal(prices[i], `The weiPrice for listing ${i} should match`);
                expect(listing.skinInfo.floatValues).to.equal(skinInfo[i].floatValues, `The skinInfo floatValues for listing ${i} should match`);
                expect(listing.skinInfo.paintSeed).to.equal(skinInfo[i].paintSeed, `The skinInfo paintSeed for listing ${i} should match`);
                expect(listing.skinInfo.paintIndex).to.equal(skinInfo[i].paintIndex, `The skinInfo paintIndex for listing ${i} should match`);
                expect(listing.stickers.length).to.equal(stickers[i].length, `The stickers length for listing ${i} should match`);
                expect(listing.weaponType).to.equal(weaponTypes[i], `The weaponType for listing ${i} should match`);
                expect(listing.priceType).to.equal(priceTypes[i], `The priceType for listing ${i} should match`);

                for (let j = 0; j < stickers[i].length; j++) {
                    expect(listing.stickers[j].name).to.equal(stickers[i][j].name, `The sticker name for listing ${i} should match`);
                    expect(listing.stickers[j].material).to.equal(stickers[i][j].material, `The sticker material for listing ${i} should match`);
                    expect(listing.stickers[j].slot).to.equal(stickers[i][j].slot, `The sticker slot for listing ${i} should match`);
                    expect(listing.stickers[j].imageLink).to.equal(stickers[i][j].imageLink, `The sticker imageLink for listing ${i} should match`);
                }
                expect(listing.status).to.equal(0, `The status for listing ${i} should be ForSale`);                
                expect(listing.seller).to.equal(await deployer.getAddress(), `The seller for listing ${i} should be the deployer`);
                expect(listing.buyer).to.equal(`0x${'0'.repeat(40)}`, `The buyer for listing ${i} should be the zero address`);
                expect(listing.buyerTradeUrl.partner).to.equal('', `The buyerTradeUrl partner for listing ${i} should be empty`);
                expect(listing.buyerTradeUrl.token).to.equal('', `The buyerTradeUrl token for listing ${i} should be empty`);
                expect(listing.buyerTradeUrl.token).to.equal('', `The buyerTradeUrl token for listing ${i} should be empty`);                
            }
    ...
    describe("Base Fee Operations", function () {
        ...
        it("should not allow unauthorized user to change the base fee", async function () {
            const newBaseFee = '7';
            await expect(tradeFactory.connect(user2).changeBaseFee(newBaseFee))
